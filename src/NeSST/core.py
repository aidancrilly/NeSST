...
available_materials = list(sm.mat_dict.keys())
...
def init_DT_scatter(Eout : npt.NDArray, Ein : npt.NDArray):
    ...
    """
    sm.mat_dict['D'].init_energy_grids(Eout,Ein)
    sm.mat_dict['T'].init_energy_grids(Eout,Ein)
    sm.mat_dict['D'].init_station_scatter_matrices()
    sm.mat_dict['T'].init_station_scatter_matrices()
...
def init_DT_ionkin_scatter(varr : npt.NDArray, nT: bool = False, nD: bool = False):
    ...
    """
    if(nT):
        if(sm.mat_dict['T'].Ein is None):
            print("nT - Needed to initialise energy grids - see init_DT_scatter")
        else:
            sm.mat_dict['T'].full_scattering_matrix_create(varr)
    if(nD):
        if(sm.mat_dict['D'].Ein is None):
            print("nD - Needed to initialise energy grids - see init_DT_scatter")
        else:
            sm.mat_dict['D'].full_scattering_matrix_create(varr)

def calc_DT_ionkin_primspec_rhoL_integral(I_E : npt.NDArray, rhoL_func=None, nT: bool = False, nD: bool = False):
    if(nT):
        if(sm.mat_dict['T'].vvec is None):
            print("nT - Needed to initialise velocity grid - see init_DT_ionkin_scatter")
        else:
            if(rhoL_func is not None):
                sm.mat_dict['T'].scattering_matrix_apply_rhoLfunc(rhoL_func)
            sm.mat_dict['T'].matrix_primspec_int(I_E)
    if(nD):
        if(sm.mat_dict['D'].vvec is None):
            print("nD - Needed to initialise velocity grid - see init_DT_ionkin_scatter")
        else:
            if(rhoL_func is not None):
                sm.mat_dict['D'].scattering_matrix_apply_rhoLfunc(rhoL_func)
            sm.mat_dict['D'].matrix_primspec_int(I_E)
...
def DT_sym_scatter_spec(I_E : npt.NDArray
    ...
    sm.mat_dict['D'].calc_dNdEs(I_E,rhoL_func)
    sm.mat_dict['T'].calc_dNdEs(I_E,rhoL_func)
    nD   = frac_D*sm.mat_dict['D'].elastic_dNdE
    nT   = frac_T*sm.mat_dict['T'].elastic_dNdE
    Dn2n = frac_D*sm.mat_dict['D'].n2n_dNdE
    Tn2n = frac_T*sm.mat_dict['T'].n2n_dNdE
...
def DT_scatter_spec_w_ionkin(I_E : npt.NDArray, vbar : float, dv : float, rhoL_func : callable,
    ...
    sm.mat_dict['T'].calc_dNdEs(I_E,rhoL_func)
    if(sm.mat_dict['D'].vvec is None):
        dNdE_nD = sm.mat_dict['D'].elastic_dNdE
    else:
        dNdE_nD = sm.mat_dict['D'].matrix_interpolate_gaussian(sm.mat_dict['D'].Eout,vbar,dv)
    if(sm.mat_dict['T'].vvec is None):
        dNdE_nT = sm.mat_dict['T'].elastic_dNdE
    else:
        dNdE_nT = sm.mat_dict['T'].matrix_interpolate_gaussian(sm.mat_dict['T'].Eout,vbar,dv)
    nD   = frac_D*dNdE_nD
    nT   = frac_T*dNdE_nT
    Dn2n = frac_D*sm.mat_dict['D'].n2n_dNdE
    Tn2n = frac_T*sm.mat_dict['T'].n2n_dNdE
...
def calc_DT_sigmabar(Ein : npt.NDArray, I_E : npt.NDArray,
    ...
    """
    sigmabar = frac_D*np.trapz(sm.mat_dict['D'].sigma_tot(Ein)*I_E,Ein)+frac_T*np.trapz(sm.mat_dict['T'].sigma_tot(Ein)*I_E,Ein)
    return sigmabar
